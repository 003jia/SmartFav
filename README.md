...
- 分类内单条书签可跨分类转移；移动、文件夹改层级和删除迁移统一使用独立目录选择页，支持搜索、面包屑和多级进入，不再遮挡列表内容。
...
- 收藏移动、文件夹跨层移动和删除迁移改为独立目录选择页，支持路径搜索、面包屑和任意层级进入；选择目标后自动返回原页面。
...
当前扩展共 18 个 JavaScript 文件。1.15.1 在 1.15.0 多级目录回归基础上，继续覆盖旧数据迁移、四层目录、同名目录冲突、递归浏览器双向同步、同网址新增/覆盖、AI 预览不落盘、恶意名称/未知 ID 拒绝、批量中断与还原入口，以及独立目录选择器的结构与导航原语等自动化回归。
