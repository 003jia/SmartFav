...
<p align="center">
  <a href="dist/SmartFav-Edge-1.14.1.zip"><strong>下载 1.14.1</strong></a>
  ·
  <a href="#安装">安装说明</a>
  ·
...
- [`SmartFav-Edge-1.14.1.zip`](dist/SmartFav-Edge-1.14.1.zip)：Manifest V3 扩展包。
...
1. 下载 `SmartFav-Edge-1.14.1.zip` 并解压。
...
- 本地分类新增“把握高 / 中 / 低”提示，并继续展示具体关键词或域名匹配依据。
- 在 SmartFav 内手动跨分类移动书签后，会询问是否“记住这次分类”。
- 只有点击“记住”才会写入 `domain:网站域名` 规则；点击“这次不用”不会改变规则。
- 当前网页分类被手动修改后，会出现“同时记住这个网站的分类”，普通分类默认勾选。
- “其他”通常是临时归类，因此默认不勾选学习，但用户仍可主动开启。
- 保存后提供“撤销记住”；撤销只恢复关键词规则，刚收藏的网页仍保留在手动选择的分类。
- 学到的域名规则直接显示在“分类文件夹”的关键词输入框中，可编辑、移动或删除。
- 同一域名已有旧规则时会先明确提示；确认后只保留新的目标分类，避免冲突。
- 新规则会用于后续插件收藏、浏览器星标自动获取和浏览器收藏夹整理，不会静默重分已有收藏。

完整版本记录见 [扩展使用文档](smartFav智能收藏夹/README.md#版本记录)。

## 后续计划

下一版优先继续完善分类学习闭环，包括浏览器移动产生的待确认建议、学习记录管理和长期撤销入口。完整数据备份、搜索去重、批量整理和更深入的可选 AI 建议随后逐步推进。
...
unzip -t dist/SmartFav-Edge-1.14.1.zip
```

1.14.1 已通过以上语法检查、扩展自动化验证和 ZIP 完整性检查。
