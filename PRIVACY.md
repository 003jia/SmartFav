...
- **同步浏览器收藏夹：**把 SmartFav 收藏写入受管理的 `SmartFav / 分类` 文件夹；用户删除已同步的 SmartFav 收藏时，删除受管理目录中的对应记录。用户在浏览器删除最后一条同网址书签时，对应 SmartFav 收藏会移入“最近删除”；如果仍有同网址浏览器副本，则不会删除 SmartFav 收藏。
